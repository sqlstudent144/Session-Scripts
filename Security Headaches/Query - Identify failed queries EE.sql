...
ADD TARGET package0.event_file (SET 
	filename = N'C:\temp\FailedQueries.xel'
...
SELECT
	...
	[XML Data].value('(/event/action[@name=''database_name'']/value)[1]','nvarchar(max)')AS [Database],
	[XML Data].value('(/event/data[@name=''message'']/value)[1]','nvarchar(max)')AS [Message],
...
FROM
	(SELECT 
		...
		FROM sys.fn_xe_file_target_read_file
					('C:\temp\FailedQueries*.xel',NULL,NULL,NULL)
	) as FailedQueries;
GO
